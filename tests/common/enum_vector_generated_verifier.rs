...
pub mod example {
    ...
    impl<'a> Verify for reader::Bag<'a> {
        fn verify(&self) -> Result {
            let tab = self._tab;
            let buf = tab.buf;
            let buf_len = buf.len();
            ...
            if Self::VT_COLORS as usize + flatbuffers::SIZE_VOFFSET <= vtab_num_bytes {
                let voffset = vtab.get(Self::VT_COLORS) as usize;
                if voffset > 0 {
                    ...
                    colors_verifier.verify_scalar_elements(1)?;
                }
            }
