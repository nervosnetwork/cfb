...
pub mod example {
    ...
    impl<'a> Verify for reader::Sensor<'a> {
        fn verify(&self) -> Result {
            let tab = self._tab;
            let buf = tab.buf;
            let buf_len = buf.len();
            ...
            if Self::VT_READINGS as usize + flatbuffers::SIZE_VOFFSET <= vtab_num_bytes {
                let voffset = vtab.get(Self::VT_READINGS) as usize;
                if voffset > 0 {
                    if voffset + 4 > object_inline_num_bytes {
                        return Err(Error::OutOfBounds);
                    }

                    let readings_verifier =
                        VectorVerifier::follow(buf, try_follow_uoffset(buf, tab.loc + voffset)?);
                    readings_verifier.verify_scalar_elements(4)?;
                }
            }
