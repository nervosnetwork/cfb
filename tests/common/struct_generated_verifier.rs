...
pub mod example {
    ...
    impl<'a> Verify for reader::Point<'a> {
        fn verify(&self) -> Result {
            ...
            if Self::VT_POSITION as usize + flatbuffers::SIZE_VOFFSET <= vtab_num_bytes {
                let voffset = vtab.get(Self::VT_POSITION) as usize;
                if voffset > 0 && voffset + 24 > object_inline_num_bytes {
                    return Err(Error::OutOfBounds);
                }
            }
